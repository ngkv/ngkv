...
pub(crate) fn serialize(t: &impl Serialize) -> Result<Vec<u8>> {
    // Currently there is no reason for serialization to fail.
    let res = bincode_options().serialize(t).unwrap();
    Ok(res)
}

pub(crate) fn serialize_into(w: impl Write, t: &impl Serialize) -> Result<()> {
    // Currently there is no reason for serialization to fail.
...
pub(crate) fn deserialize_from<T: DeserializeOwned>(r: impl Read) -> Result<T> {
    bincode_options()
        .deserialize_from(r)
        .map_err(|_| Error::Corrupted("deserialization failed".into()))
}

pub(crate) trait FixedSizeSerializable: Sized {
